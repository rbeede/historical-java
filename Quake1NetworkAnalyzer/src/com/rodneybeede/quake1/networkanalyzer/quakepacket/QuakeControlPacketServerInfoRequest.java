...
public class QuakeControlPacketServerInfoRequest extends QuakeControlPacket {
	private final NetworkProtocolVersion networkProtocolVersion;

	public QuakeControlPacketServerInfoRequest(final byte[] bytes, final int length) {
		super(bytes, length);
		
		if(!QuakeControlPacket.OPERATION_CODE.CCREQ_SERVER_INFO.equals(this.getOperationCode())) {
			this.setInvalidReason("Provided packet is not of type " + QuakeControlPacketServerInfoRequest.class.getName() + " but is instead " + this.getOperationCode());
