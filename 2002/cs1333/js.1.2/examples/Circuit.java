...
public class Circuit
{
	...
	// simulate circuit
	time = simulate();
	input.set(time+1.0,0,15); // first: set input high
	time = simulate();
	input.set(time+1.0,0,0);  // later: set input low
	time = simulate();
	input.set(time+1.0,0,15); // later: set input high
	time = simulate();
	input.set(time+1.0,0,0);  // later: set input low
	simulate();
    }
...
abstract class Component
{
    ...
    {
	Iterator i = neighbors.elements();
	while (i.hasMoreElements())
	{
	    Connection c = (Connection)i.nextElement();
...
/*
//+output
1.0ns: output now 0 volts
-- circuit stable after 1.0ns --
2.0ns: input set to 15 volts
2.8ns: output now 15 volts
3.0ns: output now 0 volts
-- circuit stable after 3.0ns --
4.0ns: input set to 0 volts
-- circuit stable after 5.0ns --
6.0ns: input set to 15 volts
6.8ns: output now 15 volts
7.0ns: output now 0 volts
-- circuit stable after 7.0ns --
8.0ns: input set to 0 volts
-- circuit stable after 9.0ns --
//-output
*/
