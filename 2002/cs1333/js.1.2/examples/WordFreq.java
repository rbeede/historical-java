...
public class WordFreq
{
	...
	for (r.skipWhite(); !r.eof(); r.skipWhite())
	{
	    ...
	    String word = r.readString();
	    for (i = 0; i < vocab.size(); i++)
	    {
		// get the association
		wordInfo = (Association)vocab.elementAt(i);
		// get the word from the association
		vocabWord = (String)wordInfo.key();
		if (vocabWord.equals(word))
		{   // match: increment integer in association
		    Integer f = (Integer)wordInfo.value();
	    ...
	    {
		vocab.addElement(
		    new Association(word,new Integer(1)));
	    }	    
	}
	// print out the accumulated word frequencies
	for (i = 0; i < vocab.size(); i++)
	{
	    Association wordInfo = (Association)vocab.elementAt(i);
	    System.out.println(
 	       wordInfo.key()+" occurs "+
               wordInfo.value()+" times.");
	}
    }
    //-implementation
}
