...
class Loan {
     public static void main (String[] args) throws IOException {
          ...
          while ( 1 == 1 ) {
               ...
               while ( dblYearRateInterest < 0.0 ) {
                    ...
                    //Convert input into a double 
                    dblYearRateInterest = new Float( strKeyInput ).doubleValue();
                    ...
                    dblYearRateInterest = Round( (double) (dblYearRateInterest / 100), 5 );
               ...
               dblMonthPay = (dblAmountLoan * (dblYearRateInterest / 12) )
                  /  ( 1 - Math.pow( (1 + (dblYearRateInterest / 12)), (double) (intYearsLoanLength * -12) ) );
               ...
               //completely paid off
               for( intPaymentNum = 1; intPaymentNum <= (intYearsLoanLength * 12); intPaymentNum++ ) {
                    //Do the calculations
                    ...
                    System.out.println( " # " + FormatNumber( intPaymentNum, 5 ) + " | $" +  FixDecimal( dblInterestPay, FormatNumber( Round( dblInterestPay, 2 ), 4 ) ) + " | $" + FixDecimal( dblAccumInterest, FormatNumber( Round( dblAccumInterest, 2 ), 4 ) ) + " | $" + FixDecimal( dblPrincipalPay, FormatNumber( Round( dblPrincipalPay, 2 ), 5 ) ) + " | $" + FixDecimal( dblAccumPrincipal, FormatNumber( Round( dblAccumPrincipal, 2 ), 5 ) ) + " | $" + FixDecimal( dblBalanceDue, FormatNumber( Round( dblBalanceDue, 2 ), 7 ) )  );
              }
     ...
     //to the 1st decimal number they would pass the value 2 for
     //intPlace (10^1 = , 1 * 5.5 = 5.5, 5.5 + 0.5 = 6.0)
     //If dblNumber = 2654 and they wanted to round it to the
     //hundreds place the would pass the value -2 for intPlace
     //(10^-2 = .01, .01 * 2654 = 26.54, 26.54 + 0.5 = 27.04, 10^-2 /
     //(int) 27.04 = 2700)
     ...
     public static double Round( double dblNumber, int intPlace ) {
          ...
          dblNumber = (double) (intCutOffNum / Math.pow( (double) 10, (double) intPlace ));
     ...
     public static String FormatNumber( double dblNumber, int intDigits ) {
          ...
          do {
               ...
               if( dblNumber < (Math.pow( (double) 10, (double) intCurDigit )) )  //Found how many digits
     ...
     public static String FormatNumber( int intNumber, int intDigits ) {
          ...
          do {
               ...
               if( intNumber < (Math.pow( (double) 10, (double) intCurDigit )) )  //Found how many digits
                    break;  //Bust out of the loop
          } while( intCurDigit <= intDigits );
     ...
     public static String FixDecimal( double dblNumber, String strNumber ) {
